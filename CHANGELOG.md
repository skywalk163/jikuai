...
## v0.29.0（2026-08-21）· 单目标轮：把「真端点」的**契约半边**真跑通，另一半照旧挂账

> WBS 见 `docs/v0.29.0-WBS.md`（M31 · W181–W185，M30 已被 v0.24.0 占用）；
> 复盘见 `docs/v0.29-复盘.md`。**没有新立 ADR**——本轮沿用
> `docs/ADR-41-规划器与NL层.md`（§8 台架不进 CI 的既定判据、§9 语义层直取旁路）。
> 契约门禁 **G10–G25 全绿**（`scripts/check_stdlib_contract.py` exit 0），**没有新增门禁**。
> 全量回归 **3366 passed / 88 skipped**（= v0.28.0 收口 3323 + 本轮新增 43）。
>
> **本轮一行产品代码没改**：`blocks_cli.py` / `planner.py` / `retrieval.py` /
> `demo_server.py` / `schema.py` 全部未动，块库仍 **145 块**，规划录像仍 **18** 份。
> 产出全是台架 + 测试 + 文档。

### 开场三问（拍板结论，决定了本轮边界）

- **范围**：只清 v0.28.0 §六.1 那一条（真实模型端点验证），不铺第二摊。
- **适配层**：**不接通用 LLM**，只验「自定义回填端点」契约。
- **部署**：只做**临时内网穿透**的本地演示，**不做**正式反代 / HTTPS。

### W181 · 参考回填端点（`tools/ai-bridge/参考回填端点.py`）

- 一个独立进程的 HTTP 回填端点，三种模式：`标量`（挑输入槽全是 `数` 的候选）/
  `录像`（逐字复用 `规划录像/` 里的人工方案）/ `幻觉`（**故意**给不在候选白名单里的块名）。
- **它是机械对拍端点，不是模型**，且这件事写进了数据而不只是文档：回填的 `模型` 字段
  恒为 `参考端点·<模式>`，录像转投时写成 `参考端点·录像转投（原：X）` 保留人工出处。
- 装不下就回 **422 + 中文理由**，**不硬凑**一个形状合法的方案。
- 鉴权：`Authorization: Bearer`，令牌**只从** `JIKUAI_PLANNER_TOKEN` 取（与 `问` 同源），
  非 ASCII 令牌**拒绝启动**；监听地址硬编码 `127.0.0.1`；GET → 405、坏 JSON → 400、
  令牌不符 → 401。
- `tests/test_v0_29_0_w181_参考回填端点.py` **19 passed**，含真 socket 往返的
  401 / 正确 Bearer / 错误 Bearer 三条。

### W182–W183 · 跨进程真跑（`tools/ai-bridge/真跑_问.py`）

**补的是「证据类型」，不是冗余覆盖。** v0.27.0 W158/W159 那 25 条测试用的是 pytest
进程内的 `HTTPServer`——共享同一个解释器、同一份 `sys.path`、同一套编码设置。拆成
两个真进程后立刻抓到两个只在跨进程才暴露的问题（见下面「必记单点」）。

5 个用例 **5/5 一致**，归档 `tools/ai-bridge/真跑记录/`：

- **录像正例**：退出码 0，真实 CSV 结果
  `M003 6376 / M001 5380 / M002 4073 / M005 3714 / M006 3618 / M004 2876 /
  M007 2459 / M008 1435`，粘合源码 **22 行**。
- **标量正例**：退出码 0。
- **幻觉块名**：跨进程被同一个校验器拒（`校验拒绝` + `白名单`）——证明 `问` 不因为
  「模型是自己调的」就旁路校验器。
- **令牌不匹配**：`HTTP 401`。
- **库外问句**：**在调端点之前**就按 `拒答建议.覆盖=False` 拒答，端点一个请求都没收到。

归档**刻意另开目录**、不进 `规划录像/`：后者是白名单可承载率的分母，混进去等于悄悄
改度量基线（v0.28.0 W175 的教训）。`test_真跑记录没混进规划录像` 断言它仍恰好 18 份。

`tests/test_v0_29_0_w182_w183_真跑台架.py` **18 passed**，其中 `test_台架不进CI`
反向 grep 门禁脚本与两条流水线，把「故意不进 CI」这个决定钉成可回归的事实。

### W184 · 临时穿透文档（`docs/安全边界.md` §6.3）

- **没有推翻** v0.28.0 的「不做公网部署」：穿透只是本地演示手段，赛题「在线访问链接」
  一栏**照旧如实空着**——填一个动态、非永久的地址属虚报。v0.28.0 那两条诚实性守卫测试
  **一个字没改**，`test_v0_28_0_w178_交付物.py` **24 passed** 照旧。
- 5 条操作要点：先设 ASCII 令牌 / **保持 `127.0.0.1`、不许改 `0.0.0.0`** /
  `ssh -R` 或 `cloudflared` / 用完停穿透 + 停服务 + 轮换令牌 / 四道闸一个不改。
- ⚠️ 明写：临时穿透**不提供任何限流**，链接**仅临时有效、不对外公开**。
- `tests/test_v0_29_0_w184_临时穿透文档.py` **6 passed**，含两条反向断言
  `交付物清单.md` 仍写着「不做公网部署」「如实空着」且不含任何本机/穿透 URL。

### 挂账口径（**这是本轮最容易被误读的地方**）

`docs/BACKLOG.md` §12.4 已把那条缺口**拆成两半**记：

- **契约端点半边：已落地**（W181–W183，5/5 一致，有归档）。证明的是**契约 + 传输**：
  真 HTTP、真 Bearer、真 401、幻觉块名跨进程被拦、端点前置拒答。
- **真实模型半边：仍挂账**。`_请求回填` 只吃**裸信封 JSON**——不剥 markdown、
  不认 OpenAI `choices[]`，接通用 LLM 需要一层适配。**这是顺延，不是销账**，
  `docs/v0.28-复盘.md` §六.1 已按 W175 先例加补记（原文一字未改）。

### 必记单点（踩过的坑）

- **`tools/ai-bridge/select.py` 遮蔽标准库 `select`**（本轮最贵一坑，排查占了大半时间）。
  在该目录起 HTTP 服务时 Python 把脚本目录塞进 `sys.path[0]`，`selectors` 于是导到这个
  ...
  修法是把脚本目录挪到 `sys.path` 末尾，且这段**必须排在 `http.server`/`socketserver`
  导入之前**：第一次修放在 import 之后**没生效**，`selectors` 已经导入并缓存了。
- **跨进程台架必须把子进程 stderr 一并归档**。起初没记，根因完全不可见；加了 `端点异常`
  字段（端点进程尾声，含 `Traceback` 就记下）之后一眼就看到了。
- **要进 HTTP 头的令牌必须纯 ASCII**。用例表里写中文令牌，端点自己的守卫拒绝启动，
  表现为「端点进程刚启动就退了」，与真 bug 同形。
- **`--top` 调小拦不住语义层直取旁路**（ADR-41 §9）：骨架块 `表载入`/`窗口`/`选取`
  不受 `top` 约束，造「候选装不下」反例要直接清空 `包[候选]`。已补
  `test_调小top拦不住语义旁路所以承载不随top缩水` 把这个行为钉住。
- **G24/G25 通过时不打印任何行**（只有 G22/G23 打结论行）。别拿输出里有没有 `G24`
  字样判成败，看进程退出码。

### 其它

- 版本号 `0.28.0 → 0.29.0` 六处投影：`src/jikuai/_version.py`（真源）+
  `editors/vscode/package.json` + `lsp/jikuai_lsp/_version.py` +
  `dap/jikuai_dap/_version.py` + 双 CHANGELOG。`pyproject.toml` 走
  `[tool.setuptools.dynamic]` 引真源、不需手改。
- `AGENTS.md` **只改第六节**（新增 `select.py` 遮蔽陷阱 + 真跑台架两条）。
  **第四节一字未动**，故按 §12.4 的判据线**不触发三臂对照重跑**。
- **覆盖率本轮未重跑**（未改产品代码），沿用 v0.28.0 的 86.7% —— 如实记，
  不当作本轮达标。

## v0.28.0（2026-08-21）· 交付与硬化：把「可以给人点的端点」和「说得清口径的报表」做出来

> WBS 见 `docs/v0.28.0-WBS.md`（W164–W180）；本轮新立正本
> `docs/ADR-42-演示端点安全模型.md`；口径正本沿用 `docs/ADR-40-制造域数据模型.md`
> （本轮扩到 §5.5 / §5.7 / §5.8），规划器正本 `docs/ADR-41-规划器与NL层.md`（新增 §9）。
> 契约门禁 **G10–G25 全绿**（`scripts/check_stdlib_contract.py` exit 0），其中
> **G24 演示端点安全契约**与 **G25 块形参名词法原子性**是本轮新增门禁。
> 全量回归 **3323 passed / 88 skipped**；覆盖率 6 个受保护文件点阈值全达标、总 **86.7%**。

> **这一轮的主线**是两件「做不好会真出事」的事：①仓库里那套 `tools/web/server.py`
> 无鉴权且 `/api/跑` 在本机进程内执行提交的极快代码（经 `蟒:` 桥 = 任意代码执行），
> 它是路线图点名的最大风险；②制造域报表里那些「两种口径都对、必须选一条」的地方，
> 选错了**代码会跑通而数字是错的**。前者靠新开一套物理隔离的安全端点 + G24 解决，
> 后者靠把口径分歧点从四处扩到五处、并让每一份产出物都显式声明自己站哪一侧。

### 演示端点 · 新开一套，不给旧的打补丁（W164–W166）

新模块 `tools/web/demo_server.py`（标准库 `http.server`，零第三方依赖），与
`tools/web/server.py` **物理隔离、不共享路由表**。三端点：`POST /演示/问`（出规划
上下文包，离线、不碰模型）/ `POST /演示/跑`（**只收 `方案`**，出现 `源码` 键直接 400
而不是忽略）/ `GET /演示/白名单`。四道闸**全部默认拒绝**：

- **Bearer 鉴权** —— Token 只从 `JIKUAI_DEMO_TOKEN` 取，未设则**拒绝启动**（不生成
  随机默认 Token 打进日志），比对走 `hmac.compare_digest`。
- **块白名单** —— 模块级 `允许块`，仅制造域。
- **`subprocess` 隔离执行 + 超时** —— 超时 504，**绝不进程内 `run_source`**。
- **数据集路径只读闸** —— `realpath` + `commonpath`。

**为什么不是给旧服务加一个「安全模式」开关**：危险能力与安全能力在同一个 handler 里
靠 flag 区分，一次配置失误就穿透，也没法审计。两套代码分开才说得清信任边界
（`docs/安全边界.md` 新增 §6 并附「怎么验证它真在守」）。

新门禁 **G24** 是**静态 4 项 + 行为 4 项两套断言**：只静态时把白名单的循环体注释掉照样
绿，只行为时把校验器改成「凡不认识的块就拒」也能全场景过而白名单其实已经没了。

**本轮不做公网部署、不给在线链接**（ADR-42 与 `赛题/chatbi/产出/交付物清单.md` 都如实
记为未做）。拿本机端点冒充在线链接，上面这些就白做了。

### 六条能力缺口逐条落地（W167–W171）

块库 **139 → 145**，制造域 **30 → 33**（新增全是纯 `.jk`，`块背衬PY数` 仍 **37**）：

- **「只出汇总表」姊妹出口**（`停线汇总表`/`计停表`、`延期汇总表`/`计延表`、
  `延期排行表`/`延榜表`）—— 0 层块出元组，而 `定序`/`摘前` 吃表，元组接不进链，
  之前得让调用方手写 `【0】`。代价是丢掉排除计数（要计数就照旧调 0 层块）。
- **`窗口补集`（`截外`）/ `多窗口`（`截多`）** —— `窗口` 一行未动。实测顺带证明：
  Q_PUB_004 换成全补集口径后 L002 升幅 `0.258857 → 0.259574`（差 0.07 个百分点），
  **两种基线下榜首都是它**，所以这条是表达能力缺口、不是那道题的错误来源；
  W155 的 reference **没为了让两个数对上而被改动**。
- **口径分歧点四处 → 五处**：第五处 = 「绝对量 vs 比率排名」（ADR-40 §5.5），两侧
  = (`能耗汇总`/`计耗`, `单车电耗现成`/`车耗现`)。它刻意用**「与」形触发词组**而非前四处
  的「或」形——§5.5 的分歧只在排名/取最值语境成立，「各车间总电耗是多少」答案唯一、
  无分歧。实测：「或」形在 15 份录像误报 3 份，「与」形 0 份。
  **WBS 预期的「会让既有录像判定翻转」被实测证伪：零翻转。**
- **语义层 42 条业务词全加 `时间锚点`**（19 条非 null / 23 条 null）。取值可用机械规则
  逐条复算，不是人工拍的。它**刻意不进协议字段集**（对回填无约束力、`_check_str` 拒
  `None`），而是缀进 `语义命中.口径说明` 尾部。G22 扩三条判：缺键 ≠ null、形状必须
  `表.列`、**锚点表必须等于本条的表**（跨表锚点会把时间窗筛到另一张表上且不报错）。
- **`邻期关联` 双去重标记**（`左行序` + `右行序`，`块.json` 0.1.0 → 0.2.0）。
  **WBS 写的「左行标识」是错的一侧**：Q_HID_002 里被乘一遍的是右表，只盖左侧盖不住。
  实测膨胀 `1599/185` → 按右侧去重 `562/65`，两榜榜首仍是 L002-焊装。
- **新门禁 G25 · 块 `.jk` 形参名词法原子性** —— 判据与导出名同一把尺（整体必须分词成
  单个 `IDENT`）。实测 288 `.jk` / 131 函数定义 / 285 形参 / 0 违规。这条门禁的由来是
  `赵维度列` 的 `列` 是内建动词，会被切成 `IDENT 赵维度` + `VERB 列`，编译成
  `计停(赵表, 赵维度, 列())` 后报「takes 2 positional arguments but 3 were given」，
  看着像模块加载器的锅。**判原子性只能走 lexer，用正则等于旁路分词器。**

### 角色视角、趋势判断、溯源（W172–W173）

- 新块 **`趋势判定`（`判势`）** —— `窗间对比`/`基线偏离` 双块**只出比率、不出升降标签，
  而且看不到日期**（窗口在上游已被 `截期` 切好），所以「参照系是哪一段」必须由第三个块
  承。`参照系` 是必填人读字符串、逐行盖进输出。判定刻意不留默认：`|r| ≤ t` → 持平
  （两端都算）、`r` 为空 → **无法判断**（绝不折进持平）、`t = 0` 合法但无默认值；
  也**不按列名自动认口径**——自动识别输入来自哪个姊妹块，等于块替调用方选了 §5.4 的一侧。
- `tools/ai-bridge/溯源.py`（子命令 `面板` / `SQL`）：**四要素溯源面板**（块名 / 版本 /
  口径声明原文 / 生成的 `.jk` 源码行）+ **等价 SQL 导出，登记 15 块、从不执行**
  （无任何 DB-API import，有源码级测试守着）。没登记就说「未登记」并给理由，**绝不猜**；
  `WITH` 全文只在全步覆盖时才给。刻意不登记三块（`质量体检` 出四元组不是表、`多窗口`
  列数依实参形状、`邻期关联` 属口径判断）。**三件交付物都是 Python 工具、不做成块**：
  它们吃「方案 + 块元数据 + 合成源码」，做成块就得让块反过来 import 自己的工具链。

### 规划器 · 语义层直取旁路（W174）

`top=8` 装不下正确方案这条挂账清了，但**清法与 WBS 原文不同**：WBS 那条「业务词 → 表 →
`描述` 提到该表的块」（一跳）实测承载率 **0.0%**，一点没动——K=8 漏掉的全是**通用管道块**
（`表载入` 20/20、`窗口` 17、`选取` 12…），它们的 `描述` 里没有表名，那条映射结构上碰不到。
**缺的不是口径块，是骨架。** 定版走两跳，且两跳都只读块作者已写下的字段：

- 实测（调优 / 留出）：K=8 承载率 **0.0% → 60.0% / 60.0%**，期望块完整命中
  **0.0% → 53.3% / 60.0%**，平均候选 8 → 16.5 / 15.6。对照「旁路关 + K=40」只有
  **20.0% / 20.0%**（候选 40）—— BACKLOG 原文记的「K=40 才 30.0%」是块库 139 的数，
  **145 下缩到 20.0%**：「调大 top」的收益随块库反向缩水，旁路不随。
- **代价写在明面上**：近边缘兄弟块诱骗率 **62.5% → 75.0%**（多骗到一条 `停线汇总`）。
  远离档拒答率 50.0% 不变。
- 另一档「无条件注入手工挑的 6 块骨架」= 调优 80% / 留出仍 60%，多出的 20 点全在调优档
  且名单是从调优档诊断里挑的 → **判过拟合、不取**。数字漂亮不是理由。
- 落点上 `retrieval.py` **只加了一个常量**（`PATH_SEMANTIC='[语义层]'`），旁路逻辑一行
  都不在检索层，`retrieve()` 永不返回它。**检索层本轮一行行为都没改，仍是永远 top-K、
  永不拒答。**

### 挂账清账（W175–W177）

- **§5.4 时序参照系三条离线录像补齐** —— 录像 15 → **18 份**，`bench_planner --只回放`
  判定一致率 **100.0%**、通过 14 / 拒 4 / 组失败 0，v0.27.0 的成功度量 **3/5 → 5/5**。
  旧 4 份被拒录像**一字未改**。两处口径必须记：新录像另起 `_v27` 后缀 id + 清单新增
  `问句id` 指回评测集原 id（录像 id 此前直接当评测集 key 用，一题只能一条）；
  **承载率分母没动**——否则 W174 刚报的 0.0% → 60.0% 就换了分母、跨轮次没法比。
- **W156–W157 的 review 六条清账，修 3 / 不修 3，无一条静默丢**。修掉的那条真漏洞是：
  候选 `输入槽` 若为字符串，`len()`/下标都不报错、只取到单个字符，`槽.get` 抛
  AttributeError 把**校验器自己打挂**，而 CLI `校验` 收的就是任意 JSON。不修的三条都写了
  「并入条件」。其中 `分歧点表` 双存从「挂账」转为**记档的设计边界**：合成一份会让门禁
  被迫带触发词、规划器被迫带断言词。
- **三臂对照 `agents-md-ab/` 从「挂 v0.28.0」改判不做（终态）**。销账前先逐段核过
  `git diff -- AGENTS.md`：本轮对第四节的改动只有事实数字、两个台架实测数、一条防误读
  注解，**「你必须遵守」六条与模块陷阱表表体一个字没动**。判据线已抄进 AGENTS.md §六
  （原先只在 BACKLOG 里，读 AGENTS.md 的人看不到判据只能凭感觉）：改**说服性文字** ⇒
  必跑；改数字 / 注解 / 口径澄清 ⇒ 不跑。**「改第四节要重跑」这条纪律一字未删。**
- **仍然挂账**：`jk 块 问 --模型 <端点>` 的真实模型端点验证，目标版本 **v0.29.0**。
  它与三臂不同——三臂是收益已被实验证明有限，这条是**从未验证过**，属真缺口，不许悄悄
  改成「不做」。

### 交付物（W178）

`赛题/chatbi/产出/交付物清单.md` 作为单一入口：六项对账 + 技术方案一页纸 + 口径审计说明
+ 评测数字分档报表 + 复现命令。两条**诚实性守卫写成了测试**（清单必须仍含「不做公网
部署 / 如实空着」与演示视频「需人做」；清单点名的每个路径与 ADR 必须真存在）。

新增 `产出/W178-运营日报/` —— 10 步四段（产量 / 停线 / 缺陷 / 能耗）的可跑日报，
`组`/`跑` 双退 0、零 `?` 占位。它**刻意只出绝对量**：不出达成率（§5.1）、不出缺陷率
（§5.3）、不出单车电耗（§5.2+§5.5），5 个 parametrize 断言钉住这一点——因为这些指标每一个
都压在口径分歧点上，随手加一个就等于替读者选了一侧，而 §5.3 两种缺陷率实测差 **57%**。
单日窗口靠 `窗口` 的闭区间取 `起 = 止`，**没有为「按日」造新块**。
2026-06-15 实测：产量 / 停线 / 缺陷三榜榜首都是 L002（162 / 97 分钟 / 17），
而**电耗绝对量榜首换成 L008 = 35066.5** —— §5.5 在日报里的样子。

### 门禁与回归

- **G24**（演示端点安全契约，静态 + 行为）、**G25**（块形参名词法原子性）两条新门禁，
  都串在 `scripts/check_stdlib_contract.py` 主流程里，不学早期 G13+ 的 `except → 跳过`。
- G10–G25 **EXIT=0**；全量 **3323 passed / 88 skipped**；覆盖率 6 个受保护文件点阈值
  全达标、总 86.7%；`python -m build` + G20 wheel 内容 + G21 三包发行元数据全绿。
