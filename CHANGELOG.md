...
## v0.27.0（2026-08-19）· 规划器层与 NL 问答：让 agent 只选口径、不算数

> WBS 见 `docs/v0.27.0-WBS.md`（W146–W162）；架构正本 `docs/ADR-41-规划器与NL层.md`；
> 口径正本沿用 `docs/ADR-40-制造域数据模型.md`。契约门禁 **G10–G23 全绿**
> （`scripts/check_stdlib_contract.py` exit 0），其中 **G23 规划器契约**是本轮新增门禁。

> **这一轮的主线**：v0.26.0 W145 实测发现，把制造域问句交给 LLM，它**写不出正确实参**
> ——根因是「选响应」的候选不告诉 LLM 每个块吃几个参数、什么类型。所以本轮造了一层
> **规划器**：`jk 块 规划` 把「问句 + 检索候选 + 语义层 + 块元数据」组装成一个带
> **输入槽/输出类型**的上下文包，LLM（就是读 AGENTS.md 的 agent）只需回填
> `{需求, 方案, 模型}`，算力与口径判断留在确定性的「校验器 → 组 → 跑」里。

### 规划器通道（W146–W152）

新增 `tools/ai-bridge/planner.py`：`build_context(需求)` 出上下文包
`{需求, 语义命中, 候选, 回填契约, 拒答建议, 分歧告警?}`，`validate_filled(回填, 包)`
用**五条硬规则**校验回填：`参数` 每步必填且长度 = 该候选输入槽数 / `块`+`领域`+`导出名`
逐字取自本包候选白名单 / 命中口径分歧点必须显式选一条（两侧同时出现也拒）/ 信封与方案
形状合法 / `--严格` 下无声明类型的共享常量不喂精确形参。规划器**不进 wheel**（ADR-41 §7，
依赖 `组`/`跑`），只在仓库内可用。

### 时序对比双块（W153–W155）

`制造/窗间对比`、`制造/基线偏离` —— 「跟上一个平级窗口比（对称）」vs「跟自身长期基线比
（不对称）」是 ADR-40 §5.4 的第四处口径分歧点，同样两块各自把口径写进 `描述` 由调用方选。

### `jk 块 规划` / `jk 块 问`（W158–W159）

- `规划`：纯离线、不碰任何模型，出上下文包。**拒答时仍退 0**（判定写在包里供决策，不是
  命令失败）。
- `问`：端到端（包 → 你给的 `--模型` 端点 → 校验 → 组 → 跑）。不给 `--模型` 就退 1、
  **不静默降级**；端点响应一律不可信，回填后走**同一个** `validate_filled`。token 只从
  `JIKUAI_PLANNER_TOKEN` 取，命令行不收 key。

### 录像回放 + 分层台架（W160）

`tools/ai-bridge/规划录像/`（15 份回填信封 + `清单.json`）+ `bench_planner.py`。CI 离线
确定性回放「校验器 → 组 → 跑」。**绿的判据是「判定与登记相符」不是「全部通过」**：15 份
里 4 份登记为该拒且真被拒（§5.1/§5.2 两侧口径并排、§5.4 参照系分歧点未选）。台架**分层
报数**——规划器层与检索层的数字互不遮盖。

### 新门禁 G23 · 规划器契约（W161）

`scripts/check_planner_contract.py`，串进主门禁。四条断言：协议字段名只从 schema 常量取
（AST 扫 `planner.py` 键位字面量）+ 九组字段集冻结 / 五条硬规则长在原处 / **行为断言**
（六个合成场景证明「缺 `参数` 即拒」这类守卫真在守）/ 录像回放全绿。静态 + 行为两套一起
才卡得住——这是 v0.22.0 主教训「守卫绿≠守卫在守」的直接落地。

### AGENTS.md 拒答边界写清（W161）

第四节新增「规划器层有个拒答判据，但它拦的不是你以为的那件事」：实测规划器层拒答对
**远离档拒下 50%、近边缘档 0%**，兄弟块诱骗率 **62.5% 与检索层同数**。原因是覆盖判定按
字面子串匹配业务词（`中位达成率` 含 `达成率` 必判覆盖）。**明确写检索层仍 0.0%、规划器
拒答不改变这一点**——别把它读成「规划器会拒答」。

### 度量（W160 台架 · 块库 139）

- 录像回放：判定一致率 **100%**，11 通过 / 4 拒、组失败 0、关键实体命中率 **75%**。
- 候选白名单可承载率：正确方案用到的块 TF-IDF 最深排第 89 名，`top=8` **装不下任何一份**
  正确方案（K=8/20/40 = 0%/0% · 0%/0% · 30%/20%，调优/留出双集），故回放用全库当白名单、
  承载率单独分档报。
- 规划器层拒答：远离 50% / 近边缘 0% / 兄弟诱骗 62.5%。

### 其它

- 版本号 0.27.0 推六处投影（G15）；块库 **139**；CI `ci.yml`/`release.yml` 门禁步骤名
  → G10-G23。
- **挂账（BACKLOG §12.4）**：三臂对照 `agents-md-ab/` 本轮**不重跑**——新增段是防误读注解
  + `bench_planner` 已跑出的确定性数字，非说服性选块指导；降级到 v0.28.0 或下次改选块指导
  语句时再跑。理由与并入条件记档，不悄悄丢。

## v0.26.0（2026-08-18）· 第七个领域：制造域的引擎层、语义层与口径层

> WBS 见 `docs/v0.26.0-WBS.md`（W130–W145）；数据模型见
> `docs/ADR-40-制造域数据模型.md`；新路线图 `docs/路线图-v0.26-v0.28.md`。
> 契约门禁 **G10–G22 全绿**（`scripts/check_stdlib_contract.py` exit 0），
> 其中 **G22 制造域口径契约**是本轮新增的第一道口径门禁。

> **块库从 112 涨到 137 块，`ALLOWED_DOMAINS` 从六域扩到七域。** 这是块库第一次
> 长出关系代数——此前 112 块全是「一个标量进、一个标量出」的算子，没有任何一块
> 能处理「一张表」。制造域的问题（产量、达成率、缺陷率、单车电耗）本质是
> chatbi：中文业务问句 → 表上的选取/连接/分组。所以本轮分三层做：**引擎层**
> 提供关系代数算子，**语义层**把中文业务词映射到表.字段，**口径层**把带业务
> 口径的指标固化成块。

### 新增领域 `制造`（W131）

`ALLOWED_DOMAINS` 六域（财务 / 数据 / 中文 / 历法 / 网络 / 工具）→ 七域，新增
**`制造`**，共 25 块。这是 ADR-15 立域规则第一次被真正走一遍。

### 引擎层 13 块（W133–W137）· 块库首次有关系代数

表载入(读表) 表元信息(表况) 选取(筛行) 投影(择字段) 排序(定序) 取前N(摘前)
连接(联表) 分组汇总(维聚) 窗口(截期) 月标(标月) 旬标(标旬) 邻期关联(邻联)
质量体检(体检)。

- **表的运行时表示统一为 `列表<字典<字符串,任意>>`**。一个表示定死，13 个算子
  才能任意串联——否则每加一个算子就要配一套适配。
- **零运行时第三方依赖不变**：CSV 走标准库 `csv`，**不引 pandas**。这条是硬约束，
  不是偏好——引一个 C 扩展依赖就等于把 v0.24.0 打包那套事故面重新打开一遍。

### 口径层 12 块（W140–W142）· 每块 `描述` 自报口径

产量汇总(计产) 达成率权重(达权) 达成率均值(达均) 缺陷汇总(计陷) 缺陷率(陷率)
单车电耗现成(车耗现) 单车电耗重算(车耗算) 能耗汇总(计耗) 延期汇总(计延)
延期排行(延榜) 停线汇总(计停) 班间对比(班比)。

注意这里刻意成对出现的块：**达成率权重 / 达成率均值**、**单车电耗现成 /
单车电耗重算**——同一个业务词在真实报表里有两种算法，口径差一档结果就不同。
按 AGENTS.md 第四节的纪律，这类分歧**不能靠一块「猜」**，只能两块各自把口径写进
`描述` 由调用方选。ADR-40 §5 把三处分歧点冻结成文。

### 语义层（W138）

新增 `src/jikuai/stdlib/blocks/制造/语义层.json`：**42 条**中文业务词 → 表.字段，
带单位、口径备注，以及「现成比率列 / 派生 / 分母」标记。有了它，「上月缺陷率」
这类问句才有确定的落点，而不是让调用方去猜哪张表哪个字段。

### 新门禁 G22 · 制造域口径契约（W144）

`scripts/check_manufacturing_contract.py`，串进主门禁 `check_stdlib_contract.py`。
四条断言：

1. **5 个预置异常恰好命中**（四层判法）——不多不少，多了说明判法过宽、少了说明
   漏检；
2. **`制造` 域每块 `描述` 含口径关键词**——口径层的价值全在描述自报口径上，
   描述漏写口径的块等于埋了一个「会跑通、结果是错的」候选；
3. **ADR-40 §5 三处分歧点各自落地**——文档写了分歧、代码只实现一侧，是最容易
   发生的漂移；
4. **语义层 `表.字段` ↔ 真实 CSV 表头零偏差**。数据集缺席时**显式 skip 而不是
   静默通过**——「守卫绿≠守卫在守」是 v0.22 那版的主教训，静默通过正是它的形状。

反例测试 19 条：每条断言都有对应的「造出违规 → 门禁必须红」用例，否则无法证明
门禁真在守。

### 新 ADR-40「制造域数据模型」（W130）

冻结 8 张表 schema、空值策略、三处口径分歧点，并实测外键完整性（**11 条 FK 检查
孤儿全 0**）。schema 先冻结再写块，是因为引擎层 13 块全部依赖表结构稳定——
反过来做等于让 25 块跟着数据集抖。

### 四档 chatbi 评测集 + `bench_chatbi.py`（W143）

`tools/ai-bridge/评测集-chatbi{,-留出,-无覆盖,-近边缘}.json`，分别 **调优 15 /
留出 5 / 无覆盖 10 / 近边缘 8** 条，**分档报数**。沿用既有纪律：留出集只当裁判、
负例必须分远离档与近边缘档两档报——只报远离档等于自欺。
