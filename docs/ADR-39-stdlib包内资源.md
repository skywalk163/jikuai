...
读写两侧都改成显式小端。W119 已落地。

#### 验证边界：真机大端**没有**验过（这是唯一残留）

本机是 x86（小端），手上没有 s390x 之类的大端环境。所以那条测试
（`tests/test_v0_13_0_w6_retrieval.py::test_向量索引_大端平台上翻转字节`）是用
`monkeypatch.setattr(sys, 'byteorder', 'big')` **骗**代码走大端分支，再断言
`byteswap()` 真被调用、结果正好是逐字节翻转值（`[1,2,256,258]` → `[256,512,1,513]`）。

这证明的是**分支逻辑正确、那行代码没被写成空操作**；**没有**证明真大端机器上整条
链路（解释器 + numpy + `array` 模块 + 文件 IO）端到端正确——真机上可能还有别处隐含
字节序假设，monkeypatch 摸不到。类比：模拟器里测过 ≠ 真机上跑过。

#### 若今后真在大端平台上出问题，从这里查

**症状**：`jk 块 选` 返回明显不相关的候选、或分数量级异常，**但不报任何错**
（字节序错不会抛异常，这是它最坏的地方）。注意先排除「降级到启发式」——
`选响应` 的 `路径` 字段是 `[神经]` 才说明真读了向量索引。
...
print(sys.byteorder)                 # 'big' 才会走翻转分支
vi = retrieval.load_vector_index()
print(vi.dim, vi.count)              # 应为 768 / 112
print(list(vi.vectors[0][:6]))       # 拿去和小端机器上同一份文件的输出比对
```

**判读**：两台机器读出的向量应当**逐个数值完全一致**（文件是同一份，格式约定全小端）。
若不一致，就是翻转分支有洞、或有别处漏改。

**代码位置**：读侧 `src/jikuai/ai/retrieval.py` 的 `load_vector_index()`
（`sys.byteorder == 'big'` → `vec.byteswap()`）；写侧
`tools/ai-bridge/generate_embeddings.py` 的 `_write_index()`（`astype('<i2')`）。
两处都有 W119 注释说明为什么。

**彻底根治的两条路**（都比现在贵，所以现在没做）：加一个 QEMU 模拟 s390x 的 CI
runner 真跑这条用例；或把 `.bin` 改成头部**自描述字节序**的格式，不再依赖
「格式约定就是全小端」这条口头契约。
