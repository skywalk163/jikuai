...
> **2026-08-15 已拍板为 v0.23.0 主轴**，方案定稿见 `docs/ADR-38-中文分词词典.md`，
> 排期见 `docs/路线图-v0.23.md`（M29 / W109-W112）。要点：
> - 词典 565 → 约 2.9 万条 = jieba `dict.txt` top20000 ∪ 现有 565 种子（**必留**）∪
...
| 条目 | 来源 | 优先级 | 目标版本 |
|------|------|--------|----------|
| 中文分词词典从 565 条扩到可用规模（约 2.9 万）—— 立项理由是**「中文」域本身**（六个域之一、`极简·极速·极中国` 的价值主张），不是为了救检索拒答（见下「非目标」）。主要工作量在**词典来源与授权链条**，不在 FMM 代码。**方案已定稿：`docs/ADR-38-中文分词词典.md`；排期 `docs/路线图-v0.23.md` M29 / W109-W112** | 第五轮拒答实测、`docs/标准库.md` §分词、`docs/ADR-38-中文分词词典.md` | 高（已拍板） | v0.23.0 主轴 |
...
2. ~~规模对 G12 幂等口径 / `frozenset` 构建耗时 / 仓库体积的影响~~ → 实测：
   2.9 万条 = 289 KB 明文，解析 + 建 `frozenset` 8.2 ms（每进程一次，
   `_PY_BACKING_CACHE` 保证）；幂等三条 AC 不受影响，前提是文件只在导入期读、
