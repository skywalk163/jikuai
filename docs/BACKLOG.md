...
| 条目 | 来源 | 优先级 | 目标版本 |
|------|------|--------|----------|
| 中文分词词典从 565 条扩到可用规模（万级）—— 立项理由是**「中文」域本身**（六个域之一、`极简·极速·极中国` 的价值主张），不是为了救检索拒答（见下「非目标」）。主要工作量在**词典来源与授权链条**，不在 FMM 代码 | 第五轮拒答实测、`docs/标准库.md` §分词 | 中（待拍板） | v0.23.0 候选 |
...
**开工前要先答的**：
1. 词典来源与 license 链条（放进仓库的数据必须可追溯授权）。
2. 词典规模对 G12 幂等口径、`frozenset` 构建耗时、仓库体积的影响。
3. 与 §4 的 L3 扩容（同为 v0.23.0 候选、同样「待需求」）抢不抢资源——两者都不是必须。
