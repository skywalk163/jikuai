...
| 远程**发布**（`POST /publish` + 服务端进程 + 写路径鉴权 + 审计） —— ADR-34 §2.1 明确本轮只做装包端读；远程写是另一个量级（真正的服务端 + 鉴权中间件 + 并发/覆盖/配额策略）。触发条件见 ADR-34 §7「出现远程发布诉求」 | `docs/ADR-34-远程HTTP注册表.md` §2.6 | 中 | v0.21.0+（需另立 ADR） |
