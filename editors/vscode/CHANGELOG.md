# 更新日志

本扩展遵循 [语义化版本](https://semver.org/lang/zh-CN/)，版本号与主包 `jikuai` 对齐。

## [0.29.0] - 2026-08-21

版本号随主包 v0.29.0 对齐。**扩展侧无代码改动** —— 本轮是单目标轮「真端点契约验证」，
产出全在 `tools/ai-bridge/`、`tests/` 与文档，**一行产品代码都没改**，扩展不涉及。

### 与写代码体感相关的变化

- **没有任何变化**：块库仍 **145 块**、`极快.选块` 检索一行没动、门禁仍是 G10–G25、
  协议字段未增未删。本轮改的是「怎么证明 `jk 块 问 --模型 <端点>` 真的能跑」，
  不是能力面。
- 新增 `tools/ai-bridge/参考回填端点.py` 是**机械对拍端点、不是模型**（回填的 `模型`
  字段恒以 `参考端点·` 开头）。它与 `真跑_问.py` 一起**故意不进** CI 与回归，
  归档在 `tools/ai-bridge/真跑记录/`。它 5/5 一致**不代表真实模型端点已验证**。
- `jikuai-lsp` / `jikuai-dap` 版本号随主包对齐到 0.29.0（三包同号发布的既定纪律）。

## [0.28.0] - 2026-08-21

版本号随主包 v0.28.0 对齐。**扩展侧无代码改动** —— 本轮主题是「交付与硬化」
（演示端点安全化 + 六条能力缺口 + 交付物），改动全在 `tools/`、`scripts/`、
`src/jikuai/stdlib/blocks/制造/` 与文档，扩展不涉及。

### 与写代码体感相关的变化（扩展透传，无需改扩展代码）

- **`极快.选块` 检索的块库涨到 145 块**（LSP 的 `workspace/executeCommand` 与 `jk 块 选`
  同一套检索），新增 6 块全在制造域：`停线汇总表` / `延期汇总表` / `延期排行表`
  （「只出汇总表」姊妹出口）、`窗口补集` / `多窗口`（时间窗表达）、`趋势判定`（升/降/
  持平 + 参照系）。
- **新增门禁 G25 会挡住一类以前只在运行期才炸的块写法**：块 `.jk` 的**形参名**现在必须
  词法原子。写 `赵维度列` 这种含内建动词的形参名，编译出来会变成
  `计停(赵表, 赵维度, 列())` 并报「takes 2 positional arguments but 3 were given」——
  报错点离病根很远。自己写块时这条同样适用，改名避开内建动词/副词即可。
- 本轮新增的**演示端点**（`tools/web/demo_server.py`）与扩展无关：它不进 wheel、不经
  LSP，扩展里看不到它。**仓库里那套 `tools/web/server.py` 照旧不可挂网**。
- `jikuai-lsp` / `jikuai-dap` 版本号随主包对齐到 0.28.0（三包同号发布的既定纪律）。

## [0.27.0] - 2026-08-19

版本号随主包 v0.27.0 对齐。**扩展侧无代码改动** —— 本轮主题是「规划器层与 NL 问答」
（新增 `jk 块 规划` / `jk 块 问`、规划器契约门禁 G23），改动全在 `tools/ai-bridge/`、
`scripts/` 与文档，扩展不涉及。

### 与写代码体感相关的变化（扩展透传，无需改扩展代码）

- **`极快.选块` 检索的块库涨到 139 块**（LSP 的 `workspace/executeCommand` 与 `jk 块 选`
  同一套检索）。新增的规划器层（`jk 块 规划`）是 `tools/ai-bridge/` 里的 CLI，**不进
  wheel、不经 LSP**，扩展里看不到它。
- `jikuai-lsp` 版本号随主包对齐到 0.27.0（三包同号发布的既定纪律）。扩展的
  `python.defaultInterpreterPath` 之类配置不受影响。

## [0.26.0] - 2026-08-18

版本号随主包 v0.26.0 对齐。**扩展侧无代码改动** —— 本轮主题是「制造域的引擎层、
语义层与口径层」（新增第七个领域 `制造`，块库 112 → 137 块），改动全在
`src/jikuai/stdlib/blocks/制造/`、`scripts/` 与文档，扩展不涉及。

### 与写代码体感相关的变化（扩展透传，无需改扩展代码）

- **`极快.选块` 现在能选到制造域的 25 块**（LSP 的 `workspace/executeCommand`
  与 `jk 块 选` 同一套检索）：13 个关系代数引擎块（读表 / 筛行 / 择字段 / 定序 /
  摘前 / 联表 / 维聚 / 截期 / 标月 / 标旬 / 邻联 / 体检 / 表况）+ 12 个带业务
  口径的指标块（计产 / 达权 / 达均 / 计陷 / 陷率 / 车耗现 / 车耗算 / 计耗 /
  计延 / 延榜 / 计停 / 班比）。这是块库第一次能处理「一张表」而不只是标量。
- **口径成对的块要自己挑**：`达权`/`达均`、`车耗现`/`车耗算` 是同一业务词的两种
  算法，候选里会同时出现，选错**不报错、只是结果不同**。判据写在每块的 `描述`
  里（口径自报），`极快.选块` 的返回里带着它。
- `jikuai-lsp` 版本号随主包对齐到 0.26.0（三包同号发布的既定纪律）。扩展的
  `python.defaultInterpreterPath` 之类配置不受影响。

## [0.25.0] - 2026-08-16

版本号随主包 v0.25.0 对齐。**扩展侧无代码改动** —— 本轮主题是「把发布准入的门禁装到真在
跑的那条流水线上」（CI 落到自建 Gitea Actions + 半自动发版流水线），改动全在
`.gitea/workflows/`、`scripts/` 与文档，扩展不涉及。

### 与写代码体感相关的变化

- **无。** 这一版没有任何语言实现或扩展功能改动，装了扩展的人升级后行为完全不变。
- `jikuai-lsp` 版本号随主包对齐到 0.25.0（三包同号发布的既定纪律）。扩展的
  `python.defaultInterpreterPath` 之类配置不受影响。

## [0.24.0] - 2026-08-15

版本号随主包 v0.24.0 对齐。**扩展侧无代码改动** —— 本轮主题是「把 `pip install jikuai`
从坏的修成能用的」，改动落在打包布局与发布链路，扩展不涉及。

### 与写代码体感相关的变化

- **`pip install jikuai` 现在真的能用了**。此前 PyPI 上的 0.4.1 是坏包（wheel 里零个
  stdlib 文件，装完 `导入 数学` 直接失败）。装扩展的人不再必须先 clone 仓库 +
  `pip install -e .`，`pip install jikuai jikuai-lsp` 就够。
- **`jikuai-lsp` 版本号从 0.15.0 跳到 0.24.0**，与主包同号发布。此前它写死在
  `lsp/pyproject.toml` 里落后主包八个版本，没法判断「哪个 lsp 配哪个 jikuai」。
  扩展的 `python.defaultInterpreterPath` 之类配置不受影响。
- **`stdlib/` 物理位置变了**：从仓库根搬到 `src/jikuai/stdlib/`。从源码开发的人若在
  设置里硬写过 stdlib 路径要改；也可以用新的 `JIKUAI_STDLIB` 环境变量指过去。

## [0.23.0] - 2026-08-15

版本号随主包 v0.23.0 对齐。**扩展侧无代码改动** —— 本轮主题是「中文分词从玩具做成
能用的」，改动落在 `stdlib/分词` 与 `tools/dict/`，扩展不涉及。

### 与写代码体感相关的变化（扩展透传，无需改扩展代码）

- **`分词` 的结果大面积变好**：词典从 565 条扩到 58713 条（jieba top50000 ∪ 现有种子
  ∪ THUOCL 财经/法律）。以前被切成单字的词现在整体成词（`值得` / `推广` / `胸有成竹`
  等）。切分基准上边界 F1 0.513 → 0.864、词级 F1 0.004 → 0.741。这条对编辑器里
  用到 `分词` 的场景直接可见。
- 词典改成外部数据文件 `stdlib/分词词典.txt`（不再内联在 `.py` 里）。扩展不直接读它，
  但 LSP 起的 Python 进程要能找到它——沿用既有约定（先 `pip install -e .`）即可。

## [0.22.0] - 2026-08-14

版本号随主包 v0.22.0 对齐。**扩展侧无代码改动** —— 本轮主题是「质量补洞 +
注册表生产化 + AOT 子集破冰」，分别落在覆盖率工具链、`tools/registry-server/`
服务端与 `tools/aot/`，扩展不涉及。

### 与写代码体感相关的变化（扩展透传，无需改扩展代码）

- **下标取值的报错文案全变了**。`赵表[9]` / `赵d["不存在"]` / `42[0]` 以前直通
  Python 英文异常，现在是中文诊断并带行列与源码行，例如
  `下标越界：列表长度为 3，下标 9 超出有效范围（-3 到 2）`、
  `键不存在：字典里没有键 「乙」`、`整数不支持下标取值`。
  这条对**编辑器里的诊断显示直接可见**（LSP 透传解释器诊断），是修 ADR-09
  违规（诊断不得泄漏 Python 实现细节）。
- **AOT 子集扩了**：含列表字面量 / 字典字面量 / 下标读的 `.jk` 现在能编成原生
  二进制。`.成员`、下标写、容器作字典键、循环体内构造容器仍报 `JK-E7001`
  并指向 `docs/ADR-37-AOT子集破冰.md`。
- **包签名信任模型变了**：TOFU pin 文件改为每行一把公钥，签名者轮换公钥后
  **老包不再集体失效**；新增 `jk 包 密钥 信任` / `jk 包 密钥 撤信`。

### 已知限制

- `.vsix` 本轮**未重新真机验证**（扩展侧零改动，沿用 v0.18.0 W57 的验证结论）。

## [0.21.0] - 2026-08-13


版本号随主包 v0.21.0 对齐。**扩展侧无代码改动** —— 本轮主题是「安全审计前置 +
远程发布闭环」，全在 `jk 包` CLI、包管理内核与新的 `tools/registry-server/`
服务端，扩展不涉及。

### 服务端能力变化（扩展透传，无需改扩展代码）

- 远程发布落地（ADR-35）：`JIKUAI_REGISTRY=https://...` 时 `jk 包 发布` 走
  `POST <base>/publish` 推到远端注册表；远程分支**强制 `--签名 <别名>`**，
  且**一律拒 `--允许覆盖`**（要重发只能升版本号）。
- 新增零依赖注册表服务端 `tools/registry-server/`：token 只存 sha256、
  token↔签名者↔包名白名单三重绑定、频次/单包/请求体三层配额、
  append-only 审计（拒绝也记、不记 token）。
- 安全审计三处加固（W86）：`.块根.json` 读侧路径校验、解压炸弹三道限额、
  HTTP 响应体上限。**踩到限额的超大包从此装不上**，默认值见 `docs/包管理.md`。

### 已知限制

- `.vsix` 本轮**未重新真机验证**（扩展侧零改动，沿用 v0.18.0 W57 的验证结论）。
  仓库里 `jikuai-vscode-0.20.0.vsix` 是上一轮的产物，未纳入版本控制。

## [0.20.0] - 2026-08-12

版本号随主包 v0.20.0 对齐。**扩展侧无代码改动** —— 本轮主题是「HTTP 远程注册表
+ 包签名 + 可信跨机分发」，全在 `jk 包` CLI 与包管理内核，扩展不涉及。

### 服务端能力变化（扩展透传，无需改扩展代码）

- 包签名与验签落地（ADR-33）：`jk 包 发布 --签名 <别名>` 用 Ed25519 签校验和；
  `jk 包 装` 自动三道检查（完整性 / 签名 / 未签名过渡告警），TOFU 信任库 pin 公钥。
- 远程 HTTP 注册表（ADR-34）：`JIKUAI_REGISTRY=https://...` 即可从远程注册表装包，
  快照走 tar.gz 传输；支持 per-dependency 注册表覆盖与 Bearer token 鉴权。

## [0.19.0] - 2026-08-12

版本号随主包 v0.19.0 对齐。**扩展侧无代码改动** —— 本轮主题是「块包一体 +
生态冷启动」（第三方块能通过包管理装、能被检索/粘合用），全在服务端与
`jk 包`/`jk 块` CLI，扩展通过 `vscode-languageclient` 透传即可。

### 服务端能力变化（扩展透传，无需改扩展代码）

- 块包桥接落地（ADR-32）：`jk 包 装` 一个携带块的第三方包后，块能被
  `scan_blocks` / 检索 / 粘合发现——发现、执行、检索三根块根系统同时闭合。
- 步骤协议新增可选字段 `命名空间`：第三方块的导入路径从此带命名空间段
  （`从 blocks.<命名空间>.<领域>.<块> 导入 X`），LSP `极快.选块` 生成的方案
  会带上它。内置块无此字段，行为一字不变。

## [0.18.0] - 2026-08-11


版本号随主包 v0.18.0 对齐。**扩展侧无代码改动** —— 本轮 LSP 增强
（W54 多根 `definition`、W56 `_token_at` 走 lexer）全在服务端，扩展通过
`vscode-languageclient` 透传即可。

### 服务端能力变化（扩展透传，无需改扩展代码）

- `textDocument/definition` 的块路径解析扩到多根 workspace（W54）。此前只查
  `blocks_root()` 与文档自身目录，多根工程里跨根跳转会失败。
- `_token_at` 改为优先走 JiKuai lexer 分词（W56）。此前 `定义赵共享` 这种
  「关键字紧贴标识符」形态会被当成一个整体 token，**从定义处发起 rename
  拿不到符号**；现在 `定义`(KEYWORD) 与 `赵共享`(IDENT) 正确切开。

### 明确不做

- `textDocument/codeAction`：v0.18.0 W53 以 `docs/ADR-31-不做codeAction.md`
  **正式关闭**（四轮复审后结论：14 个诊断码无一满足「唯一机械修复」、唯一候选
  用例已被 `极快.选块` 覆盖、四轮零社区诉求）。重开条件见 ADR-31 §5。
  此前四个版本的 CHANGELOG 都写「留待下一轮复审」，本轮起不再逐轮挂账。

### 已知限制

- `.vsix` **已真机验证通过**（v0.18.0 W57）。用户验证环境：Node 工具链 + VS Code
  + `pip install -e .` + `pip install -e lsp\`。打包 `vsce package` 成功产出
  `jikuai-vscode-0.18.0.vsix`（14.63 KB）；安装后 LSP 拉起、hover / completion /
  definition / references / rename / signatureHelp / `极快.选块` 七项能力可见。
  两个 vsce warning（缺 `repository` 字段 + 缺独立 LICENSE 文件）不影响功能。

## [0.17.0] - 2026-08-11

版本号随主包对齐。本条目为 v0.18.0 W58 回归时补录——v0.17.0 发布时 `package.json`
已升到 `0.17.0`，但本 CHANGELOG 漏更（G15 只校验 pyproject/根 CHANGELOG/package.json
三处，未覆盖扩展 CHANGELOG），由 `test_changelog_has_current_version_entry` 逮出。

### 扩展侧变化

- 版本号随主包 v0.17.0 对齐（LSP 服务端本轮上线 `textDocument/references` 与
  `textDocument/rename`，扩展通过 `vscode-languageclient` 透传，无需扩展侧改代码）。

### 已知限制

- 沿用 v0.16.0：`.vsix` 仍未在装了 Node 工具链的机器上真机验证；四点 DoD
  （诊断 / hover / F12 / 选块）为「代码就绪待人工验证」。v0.18.0 W57 处理。

## [0.16.0] - 2026-08-10

版本号随主包对齐（此前长期停在 `0.6.0`，与主包 v0.7.0-v0.15.0 脱节九个版本）。G15 门禁现已校验本文件版本号与主包一致。

### 新增

- 命令面板 `极快: 选块`：`Ctrl+Shift+P` 唤起 → 输入框收需求 → 经 LSP
  `workspace/executeCommand: 极快.选块` 检索 → QuickPick 展示候选
  （名称 / 领域 / L层级 / 分数 / 描述）→ 选中后把
  `从 blocks.<领域>.<块名> 导入 <导出名>。` 插入编辑器光标处（无活动编辑器时
  退回复制到剪贴板）。命令**无条件注册**，LSP 未启动时给出可操作提示，不静默失败。
- `build.ps1` 一键打包脚本：前置检查 node/npm → `npm install` →
  `npm run compile` → `npx @vscode/vsce package`，中文输出 + 失败可操作提示。
- 完整安装教程与常见问题见仓库 `docs/LSP-使用.md`。

### 已知限制

- 命令面板选块插入的 `导入 <导出名>` 段：LSP 候选协议当前不含「导出名」字段，
  块目录名与真实导出名不同时先以块名兜底，需手动订正（已回传架构侧补协议）。
- **`.vsix` 首版随发布未产出**：主开发机无 Node.js 工具链，`build.ps1` 与
  `extension.ts` 为纸面产物未经真实执行。装了 Node 的机器上 `cd editors/vscode; .\build.ps1`
  一步出包；四点 DoD（诊断/hover/F12/选块）为「代码就绪待人工验证」。

## [0.6.0] - 2026-08-08

首个版本（M5-P2 支线：VS Code 扩展）。

### 新增

- 注册 `jikuai` 语言，关联 `.jk` 后缀。
- TextMate 语法高亮（`source.jikuai`）：注释（`#` / `--`）、字符串（`"..."` / `"…"`）、
  双字关键字、内建动词、副词（皆/只/归）、常量（真/假/空）、数字（阿拉伯 + 中文）、
  人民币字面量（￥/¥）、内建类型名。
- 通过 `vscode-languageclient` 以 stdio 方式拉起 `python -m jikuai_lsp`，
  提供实时诊断 / 补全（`.` `，` 触发）/ hover。
- 语言配置：中文/半角括号对的自动闭合与环绕，行注释 `--`。
- 配置项 `极快.pythonPath`（默认 `python`）、`极快.lsp.enabled`（默认 `true`）。
- LSP 启动失败时降级为「仅语法高亮」，并提示 `pip install -e lsp/`。

### 边界

- 暂不含调试（断点 / 单步），规划于 M6。
