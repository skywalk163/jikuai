...
PATH_NEURAL = '[神经]'
PATH_HEURISTIC = '[启发式]'
#: v0.28.0 W174：规划器层的**语义层直取旁路**产出的候选带这个标签。
#: 它**不是**本模块的检索路径——`retrieve()` 永远不会返回带此标签的 `Hit`，
#: 旁路整个实现在 `tools/ai-bridge/planner.py`，`jk 块 选` 一行未改。
#: 常量放在这里只为让三个 `路径` 取值同源：散成两处必然有一天对不上。
PATH_SEMANTIC = '[语义层]'
