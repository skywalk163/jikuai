...
# ---------------------------------------------------------------------------
# 断言 3 第四处（ADR-40 §5.4 时序参照系，v0.27.0 W154-W155 增补）
#
# 新增冻结表条目必须自带反例（v0.18.0 W55 起的规矩）：只测正例的门禁等于没有门禁，
# 这也是 v0.22.0「守卫绿 ≠ 守卫在守」那条主教训的落点。三类反例：
# 缺一侧块 / 两块互不点名 / 口径自述被抹掉。
# ---------------------------------------------------------------------------

def test_反例_第四处缺基线偏离那一侧(tmp_path):
    """只做 `窗间对比` 不做 `基线偏离`，正是 Q_PUB_004 会答错的那种缺位：
    选块的人只看见「跟上一个窗口比」，会把一次性波动当成常态。"""
    块根 = _拷块根(tmp_path)
    shutil.rmtree(块根 / "制造" / "基线偏离")
    块表 = G22.收集块(块根)

    问题, 过 = G22.校验分歧点(块表)
    assert 过 is None
    assert any("基线偏离" in p and "缺块" in p for p in 问题), 问题


def test_反例_第四处两块互不点名(tmp_path):
    """把 `窗间对比` 描述里对 `基线偏离` 的点名换成含糊说法。"""
    块根 = _拷块根(tmp_path)
    元 = 块根 / "制造" / "窗间对比" / "块.json"
    with 元.open("r", encoding="utf-8") as f:
        数据 = json.load(f)
    数据["描述"] = 数据["描述"].replace("基线偏离", "另一个时序块")
    with 元.open("w", encoding="utf-8") as f:
        json.dump(数据, f, ensure_ascii=False, indent=2)
    块表 = G22.收集块(块根)
...
def test_反例_基线偏离抹掉口径自述(tmp_path):
    """`相对自身基线` / `不是横向比较` 这两句就是本块的口径声明。
    抹掉后它与 `窗间对比` 在描述上无法区分——照错了那侧用会跑通、结果是错的。"""
    块根 = _拷块根(tmp_path)
    元 = 块根 / "制造" / "基线偏离" / "块.json"
    with 元.open("r", encoding="utf-8") as f:
        数据 = json.load(f)
    数据["描述"] = 数据["描述"].replace("相对自身基线", "比一比")
    with 元.open("w", encoding="utf-8") as f:
        json.dump(数据, f, ensure_ascii=False, indent=2)
    块表 = G22.收集块(块根)
    ...
    问题, 过 = G22.校验分歧点(块表)
    assert 过 is None
    assert any("基线偏离" in p and "口径自述" in p for p in 问题), 问题


def test_正例_第四处两侧齐且互相点名():
    """对照上面三条反例：真实块库里第四处必须是绿的。"""
    块表 = G22.收集块(真实块根)
    问题, 过 = G22.校验分歧点(块表)
    assert 问题 == [], 问题
    assert 过 is not None and "时序对比参照系" in 过, 过
