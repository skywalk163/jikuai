...
def test_bench一次跑出四档():
    ...
    # 27 → 30：v0.28.0 W167 加了三个「只出汇总表」姊妹块（`停线汇总表` / `延期汇总表` /
    #          `延期排行表`，BACKLOG §12.3 缺口 1 + 6）。
    # 30 → 32：v0.28.0 W168 加了 `窗口补集` / `多窗口`（BACKLOG §12.3 缺口 2）。
    # 32 → 33：v0.28.0 W172 加了 `趋势判定`（把「上升/下降/持平 + 参照系」落进输出；
    #          `窗间对比`/`基线偏离` 只出比率、且看不到日期，承不了这条 DoD，见 ADR-40 §5.8）。
    # 用等值而非下界，理由同 `块背衬PY数`：块数变了就该有人来改这个数并解释一句。
    assert 报告['制造域块数'] == 33, '制造域应为 33 块，实为 %s' % 报告['制造域块数']
