...
def test_分歧点表与G22门禁的块名集合一致():
    ...
    # 4 → 5：v0.28.0 W169 增补 §5.5 绝对量 vs 比率排名。这个数写死不写下界，
    # 理由同两张表要一致：加一处就该有人来改它并同步两边。
    assert len(planner.分歧点表) == len(门禁.分歧点表) == 5
    assert 实际 == 期望


def test_每处分歧点只用一种触发形式():
    """`触发词`（或）与 `触发词组`（与）互斥，不许一处同时挂两个键。

    两个都在时 `_命中分歧点` 只看 `触发词组`，另一个会变成看得见摸不着的死配置
    —— 那种「改了没反应」最难查。
    """
...
def test_第五处只在排名语境下触发():
    """★ 防「守卫绿着但从没被考过」。

    §5.5 用「与」形触发是 W169 的一处刻意偏离：绝对量 vs 比率的分歧**只在排名/
    取最值时成立**。所以这一条既要证明它在排名问句上真的响，也要证明它在纯取值
    问句上**不**响 —— 后者响了就是误报，会逼使用者给一个本来只有一个正确答案的
    问题硬选口径。
    """
    第五 = [处 for 处 in planner.分歧点表
            if 处['名'].startswith('绝对量 vs 比率排名')]
    assert len(第五) == 1
    处 = 第五[0]
    ...
    for 问句 in ('哪条产线电耗最高？',
                 '2026年6月各车间能耗排名',
                 '哪个车间最费电'):
        assert planner._命中分歧点(问句, 处) is True, 问句

    for 问句 in ('各车间2026年6月总电耗是多少？',
                 '生成2026年6月运营日报，包含产量、质量、能耗和订单延期概览。',
                 'L002产线6月中旬能耗异常是否与产量增加完全一致？'):
        assert planner._命中分歧点(问句, 处) is False, (
            '%s：这不是排名问句，§5.5 不该响' % 问句)
...
def test_反例3e_第五处绝对量与比率排名真的会咬人():
    """★ W169 · 证明 §5.5 不是「绿着但从没被考过」的摆设。

    这条**不手搓告警**：走真实的 `_分歧告警('哪条产线电耗最高？')`，再走真实的
    规则 3。它同时钉住三件事——排名问句必出 §5.5 告警、两侧同选拒、一侧不选拒、
    恰选一条过。

    §5.5 在 15 份既有录像上一次都不触发（W169 实测），所以**只有这条测试能证明
    这道闸在守**。删了它，那处分歧点等于没有。
    """
    告警 = planner._分歧告警('哪条产线电耗最高？')
    名字 = [警[planner._F分歧点] for 警 in 告警]
    assert any(名.startswith('绝对量 vs 比率排名') for 名 in 名字), 名字

    表型 = '列表<字典<字符串,任意>>'
    包 = _包([_候选('能耗汇总', 导出名='计耗',
                    槽=(('能耗表', 表型), ('维度列', '列表<字符串>'))),
              _候选('单车电耗现成', 导出名='车耗现',
                    槽=(('能耗表', 表型), ('维度列', '列表<字符串>')))],
             告警=告警)
    ...
    两侧同选 = _回填([
        schema.make_step('能耗汇总', '制造', '计耗', 参数=['赵表', '赵维度']),
        schema.make_step('单车电耗现成', '制造', '车耗现', 参数=['赵表', '赵维度']),
    ])
    合 = '\n'.join(planner.validate_filled(两侧同选, 包))
    assert '同时出现' in 合 and '必须显式选一条' in 合
    assert '能耗汇总' in 合 and '单车电耗现成' in 合

    包无关 = _包([_候选('排序', 导出名='定序', 槽=(('表', 表型),))], 告警=告警)
    一侧不选 = _回填([schema.make_step('排序', '制造', '定序', 参数=['赵表'])])
    assert '一个都没选' in '\n'.join(planner.validate_filled(一侧不选, 包无关))

    恰一条 = _回填([schema.make_step('能耗汇总', '制造', '计耗',
                                     参数=['赵表', '赵维度'])])
    assert planner.validate_filled(恰一条, 包) == [], '选定绝对量口径就该放行'
...
# --- W156-W157 review 清账（v0.28.0 W176）------------------------------------

def test_C004_畸形上下文包不再把校验器打挂而是回一句人话(表载入候选):
    """W157 review C004：CLI `校验` 收任意 JSON，畸形候选能走到 `槽表[k].get(...)`。

    `输入槽` 是字符串时 `len()`/下标都不报错，取到的是**单个字符**，`.get` 抛
    AttributeError——校验器自己崩掉，调用方看到的是 `planner.py` 的行号。修法是先拿
    `validate_context_envelope` 校参照包，fail-closed。
    """
    包 = _包([表载入候选])
    包['候选'][0]['输入槽'] = '不是数组'
    回填 = _回填([schema.make_step('表载入', '制造', '读表', 参数=['赵路径'])])
    理由 = planner.validate_filled(回填, 包)
    assert 理由, '畸形包必须拒，不许崩也不许放行'
    assert any('不是合法的规划上下文包' in r for r in 理由)


def test_C006_须显式选一条为False时只放过一侧都没选那一支():
    """W157 review C006：那个字段此前无人消费，等于协议里挂了句说了不算的话。

    语义是「两侧在本数据集恒等值、可任选」，所以「不选」放过；但**两侧同时出现照旧拒**
    ——一份方案里摆两个口径是两个答案，与数值等不等值无关。
    """
    表型 = '列表<字典<字符串,任意>>'
    告警 = [schema.make_divergence_warning(
        '测试用等值口径', ['甲块', '乙块'], '本数据集恒等值', 须显式选一条=False)]
    包 = _包([_候选('甲块', 导出名='走甲', 槽=(('表', 表型),)),
              _候选('乙块', 导出名='走乙', 槽=(('表', 表型),)),
              _候选('排序', 导出名='定序', 槽=(('表', 表型),))],
             告警=告警)
    一侧都没选 = _回填([schema.make_step('排序', '制造', '定序', 参数=['赵表'])])
    assert planner.validate_filled(一侧都没选, 包) == [], '标了可任选就不该逼人选'
    两侧同选 = _回填([
        schema.make_step('甲块', '制造', '走甲', 参数=['赵表']),
        schema.make_step('乙块', '制造', '走乙', 参数=['赵表']),
    ])
    assert any('同时出现' in r
               for r in planner.validate_filled(两侧同选, 包)), \
        '可任选 ≠ 可以两个一起摆'


def test_C008_CLI异常收敛成一句中文与退1而不是裸栈(tmp_path, capsys):
    """W157 review C008/M001：`planner.main` 此前无 try/except，读不到文件吐 traceback。

    退 1 是本仓「输入错误」的约定；`SchemaError`/`JSONDecodeError` 都在 `ValueError`
    下，所以一条 except 盖住三类。
    """
    assert planner.main(['校验', str(tmp_path / '没有.json'),
                         str(tmp_path / '也没有.json')]) == 1
    assert '错：' in capsys.readouterr().err

    坏 = tmp_path / '坏.json'
    坏.write_text('{不是合法JSON', encoding='utf-8')
    assert planner.main(['校验', str(坏), str(坏)]) == 1
    assert '错：' in capsys.readouterr().err
